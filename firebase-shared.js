...
export async function removeStudentFromClass(studentId) {
    try {
        ...
        await setDoc(docRef, { classId: null, className: null }, { merge: true });
        return { success: true };
    } catch (error) {
        return { success: false, error: "Öğrenci sınıftan çıkarılamadı." };
    }
}

export async function sendClassChatMessage(classId, senderId, senderName, text, role) {
    try {
...
export function listenClassChat(classId, callback) {
    const q = query(collection(db, "classChats"), where("classId", "==", classId));
    return onSnapshot(q, (snapshot) => {
        const messages = [];
        snapshot.forEach(doc => messages.push({ id: doc.id, ...doc.data() }));
        
        // Frontend'de zamana göre sırala (eskiden yeniye)
        messages.sort((a, b) => {
            const timeA = a.createdAt ? a.createdAt.toMillis() : Date.now();
            const timeB = b.createdAt ? b.createdAt.toMillis() : Date.now();
            return timeA - timeB;
